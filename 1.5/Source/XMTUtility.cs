...
namespace Xenomorphtype
{
    public class XMTUtility
    {
        ...
        public static bool IsPawnInSurgery(Pawn pawn, out Pawn surgeon, out float progress, out Bill_Medical medicalBill)
        {
            surgeon = null;
            medicalBill = null;
            progress = -1;
            ...
            if (!pawn.Map.reservationManager.IsReserved(pawn))
            {
                return false;
            }

            pawn.Map.reservationManager.TryGetReserver(pawn, Faction.OfPlayer, out surgeon);
            ...
            JobDriver_DoBill SurgeryDriver = surgeon.jobs.curDriver as JobDriver_DoBill;

            if (SurgeryDriver == null)
            {
                surgeon = null;
                return false;
            }

            if (SurgeryDriver.ticksSpentDoingRecipeWork <= 0)
            {
                surgeon = null;
                return false;
            }

            Bill_Medical MedicalBill = surgeon.CurJob.bill as Bill_Medical;
            ...
            if (!MedicalBill.recipe.IsSurgery)
            {
                surgeon = null;
                return false;
            }
            ...
            {
                if (MedicalBill == pawn.BillStack[i])
                {
                    medicalBill = MedicalBill;
                    progress = SurgeryDriver.ticksSpentDoingRecipeWork;
                    return true;
                }
            }
        ...
        public static Thing SearchRegionsByFilterForFirst(Region region, ThingFilter filter, Pawn pawn)
        {
